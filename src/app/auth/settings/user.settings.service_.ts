...
import { FormListSettings } from 'jetti-middle';
...
export class UserSettingsService implements OnDestroy {
  ...
  copySettings(kind: kind) {
    let newSettings: IUserSettings;
    switch (kind) {
      case 'filter':
        newSettings = {
      ...
      .subscribe(settings => {
        if (settings) {
          ...
          this._userSettings$.next({
            ...
            selected: settingsByKind.columns.find(e => e.selected) ?
              settingsByKind.columns.find(e => e.selected) : settingsByKind.columns.length ?
                settingsByKind.columns[0] : defaultSettings
          });
          this._userSettingsFilter$.next({
            settings: [...settingsByKind.filter, defaultSettings],
            default: defaultSettings,
            selected: settingsByKind.filter.find(e => e.selected) ?
              settingsByKind.filter.find(e => e.selected) : settingsByKind.filter.length ?
                settingsByKind.filter[0] : defaultSettings
          });
  ...
  deleteSelectedSettings(kind: kind) {
